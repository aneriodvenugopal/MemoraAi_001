...
38. **SaaS Admin Onboarding Wizard + Business Editor (Feb 2026)** — Added "+ Register Business" button on `/saas-admin` dashboard that opens a premium 3-step wizard (`OnboardBusinessWizard.js`): Step 1 Business (name, category dropdown from `/api/memoraai/saas-admin/business-categories`, city, plan, logo URL), Step 2 Owner (name, phone, email — auto-generated password), Step 3 WhatsApp (optional WABA ID, Phone Number ID, phone, Meta permanent token, business name). Final step shows credentials card (phone/password/login URL) with Copy buttons + inline "Verify WABA" ping to Meta Graph. Added **Edit** button per tenant row → `EditBusinessModal.js` (tabs: Profile / WhatsApp / Security) letting admin update name/category/logo/plan, paste/replace WABA creds (raw token never exposed — only masked), run Meta verification, and reset owner password. Backend: new endpoints on `memoraai_saas_admin.py` — `POST /tenants` (one-shot tenant+user+optional WABA creation), `PUT /tenants/{id}` (profile update), `POST /tenants/{id}/waba` (save creds), `POST /tenants/{id}/waba/verify` (Meta Graph ping), `POST /tenants/{id}/reset-password`, `GET /business-categories`. Existing `GET /tenants/{id}` now masks `access_token`. E2E verified: admin creates business in ~60s → owner logs in immediately with generated password → uploads content → AI responds. Test data automatically cleaned up.

39. **Dynamic Business Categories Registry (Feb 2026)** — Replaced the static `CATEGORY_CONFIGS` dict with a MongoDB-backed `memoraai_category_registry` collection that SaaS admin can fully CRUD at runtime. On first boot, `services/category_registry.py::ensure_seeded()` copies the 7 original categories into DB (marked `is_system: true` — soft-deleted instead of hard-deleted). All consumers (`memoraai_waba.py` template generator, `memoraai_categories.py` select/set-primary, onboarding wizard dropdown) now read from DB. New admin endpoints on `/api/memoraai/saas-admin/categories`: GET (list + services), POST (create), PUT /{slug} (update), DELETE /{slug}, plus nested service CRUD at `/categories/{slug}/services` and `/categories/{slug}/services/{id}`. New UI page `/saas-admin/categories` (`CategoriesAdmin.js`) — tree view per category with inline Add/Edit/Delete for services, Pause/Activate toggle, and a "Categories" header button on the SaaS Admin dashboard. **Verified**: admin adds "Software / IT Services" via API → appears in onboarding dropdown instantly, services can be added/edited/deleted, category pause hides from tenant dropdown.
